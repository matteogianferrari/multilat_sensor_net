...
from multilat_sensor_net.target import TargetObject, TargetService, TargetUpdater
import numpy as np
...
class TargetController:
    ...
    This class encapsulates the creation and coordination of the 'TargetObject', 'TargetService',
    ...
    Attributes:
        domain_obj: A TargetObject instance that encapsulates the target's state.
    ...
    def __init__(self, socket_addr: str, path_file: str, freq: float, loop_path: bool = False) -> None:
        ...
        """
        # Domain object
        self.domain_obj = TargetObject(start_pos=np.array([0., 0., 0.]))

        # gRPC service
        self.service = TargetService(target_ref=self.domain_obj, socket_addr=socket_addr)

        # Position updater
        self.updater = TargetUpdater(target_ref=self.domain_obj, path_file=path_file, freq=freq, loop_path=loop_path)
